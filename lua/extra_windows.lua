return function(config)
  ...
  -- TODO: check for useless functions, opening windows differ from each other, does that makes sense?
  local windows_state = {
  ...
  local function open_left_margin_window(available_columns)
    ...
    vim.bo[buf].bufhidden = "wipe" -- TODO: duplicate
  ...
  -- TODO: is get_non_extra_window_ids() needed?
  ...
  vim.api.nvim_create_autocmd({"VimEnter", "VimResized", "BufEnter", "BufDelete"}, {
    callback = function(args)
      local available_columns = calculate_available_columns() -- TODO: this function call be done later

      -- The "VimEnter" event should close the 'buffers floating window.'
  ...
  vim.api.nvim_create_autocmd({"WinNew", "WinClosed"}, {
    callback = function(args)
      ...
      vim.schedule(function()
        if ids_count > 1 then
          vim.api.nvim_exec_autocmds("VimResized", {}) -- TODO: comment
